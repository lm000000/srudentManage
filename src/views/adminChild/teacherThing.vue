<template>
  <div class="studentNum" v-if="data.length!=0">
    <div class="title">首页 > {{ $route.query.name }}</div>
    <div class="content">
        <tables>
              <td slot="name">姓名</td>
              <td slot="password">7月</td>
              <td slot="other1">8月</td>
       
          <tr slot="main" v-for="(item,index) in data" :key="index">
                <td slot="indexs">{{index}}</td>
                <td slot="names">{{item.contentLeft}}</td>
                <td slot="passwords">{{item.contentRight}}</td>
